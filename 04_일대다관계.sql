...
FOREIGN KEY(category_id) REFERENCES tbl_category(category_id)  #외래키 설정: 현재 테이블의 category_id(외래키)는 카테고리 테이블의 category_id 칼럼이 가진 값만 가질 수 있습니다.
  ON UPDATE CASCADE                                             #부모테이블의 PK가 수정되면 자식 테이블의 FK가 함께 수정됩니다.
  ON DELETE CASCADE                                             #부모테이블의 PK가 삭제되면 자식 테이블의 FK가 함께 삭제됩니다.
) ENGINE=InnoDB;

#제품의 이미지를 등록하고자 합니다.
#이미지 아이디, 제품 아이디, 이미지 주소, 메인 이미지 여부, 최초 등록 일시 정보를 저장할 수 있도록 
#tbl_product_image 테이블을 만들고 필요한 관계를 맺어보세요
...
FOREIGN KEY(product_id) REFERENCES tbl_product(product_id)
  ON UPDATE CASCADE                                              #부모테이블의 PK가 수정되면 자식 테이블의 FK가 함께 수정됩니다.
  ON DELETE SET NULL                                             #부모테이블의 PK가 삭제되면 자식 테이블의 FK가 함께 NULL됩니다.
) ENGINE=InnoDB;
