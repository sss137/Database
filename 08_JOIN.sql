...
2. Driven Table
    1) Driven Table에 의해서 참조되는 테이블
    2) 조인 조건에서 사용하는 칼럼 중 FK를 가진 테이블
    3) 일반적으로 행(ROW)이 많은 테이블
    
Optimizer(옵티마이저)
1. 어떤 테이블을 Drive Table로 사용할 것인지 선택합니다.
2. 테이블 크기, 인덱스 유무, 조건절의 선택도(조건 필터링 결과가 적을 것으로 예상되는 테이블을 선정) 등을 이용해서 Drive Table을 선택합니다.
3. 과거 DB들은 FROM절에 작성한 테이블을 Drive Table로 인식하기도 했습니다.
*/
...
3. 어느 쪽 테이블의 모든 데이터를 항상 포함할지에 따라 다음 세 가지로 나뉩니다.
   1) LEFT OUTER JOIN: JOIN절 왼쪽에 있는 테이블의 모든 데이터를 항상 조회합니다.
   2) RIGHT OUTER JOIN: JOIN절 오른쪽에 있는 테이블의 모든 데이터를 항상 조회합니다.
   3) FULL OUTER JOIN: JOIN절 양쪽에 있는 테이블의 모든 데이터를 항상 조회합니다.
*/

#모든 직원들의 부서아이디, 부서명, 직원아이디, 직원명을 조회하기(단, 직원이 근무하지 않는 부서 정보도 조회하기)
SELECT d.dept_id, d.dept_name, e.emp_id, e.emp_name
FROM tbl_department d
LEFT OUTER JOIN tbl_employee e
ON d.dept_id = e.dept_id;

#모든 직원들의 부서아이디, 부서명, 직원아이디, 직원명을 조회하기(단, 부서가 없는 직원 정보도 조회하기)
SELECT d.dept_id, d.dept_name, e.emp_id, e.emp_name
FROM tbl_department d
...
SELECT dept_location, COUNT(*)      
FROM tbl_department d
INNER JOIN tbl_employee e
ON d.dept_id = e.dept_id
GROUP BY dept_location;                   #GROUP BY로 명시한 것만 조회 가능

#4. 각 부서별 직원 수 조회하기(근무 중인 직원이 없으면 0으로 표시하기)
SELECT dept_name, COUNT(emp_id)           #직원 수를 구하기 위해서 모든 직원이 값을 가지고 있는 emp_id(PK)를 COUNT 함수에 전달
FROM tbl_department d
LEFT OUTER JOIN tbl_employee e
ON d.dept_id = e.dept_id
GROUP BY d.dept_id, dept_name;            #id까지 묶는 이유는 동명이인 같은 경우 방지